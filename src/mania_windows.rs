...
//!
//! [`crate::sunny`] only ever computed the GREAT window, which is also all that
...
/// [`crate::sunny::is_classic`], so that the windows agree with the difficulty
...
pub fn hit_windows(
    ...
    let raw = if classic {
        classic_windows(od, map.is_convert)
    } else {
        lazer_windows(od)
    };
...
mod tests {
    ...
    #[test]
    fn great_matches_existing_sunny_implementation() {
        // The GREAT window must stay bit-identical to sunny's current
        // get_hit_window_300 so that switching over cannot shift any SR.
